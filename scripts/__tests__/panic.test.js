...
vi.mock("../utils.js", async () => {
    ...
    function appendAudit(type, action, details = {}, success = true) {
        ensureDir();
        const entry = {
            id: randomUUID(),
...
describe("Panic Button", () => {
    ...
    describe("setup", async () => {
        ...
        it("should reject more than 5 actions", async () => {
            ...
            saveConfig(config);
            expect(() => setup("sixth", "11111111111111111111111111111111", "CODE5")).toThrow(/Maximum 5/);
        });
        ...
        it("should reject duplicate labels", async () => {
            vi.spyOn(console, "log").mockImplementation(() => { });
            setup("wife", "11111111111111111111111111111111", "ALPHA");
            expect(() => setup("wife", "11111111111111111111111111111111", "BETA")).toThrow(/already exists/);
        });
    });
    ...
    describe("audit trail", async () => {
        ...
        it("should log setup and revoke to audit", async () => {
            ...
            const panicEntries = entries.filter((e) => e.type === "panic");
            expect(panicEntries).toHaveLength(2);
            expect(panicEntries.find((e) => e.action === "setup")).toBeTruthy();
            expect(panicEntries.find((e) => e.action === "revoked")).toBeTruthy();
        });
    });
});
