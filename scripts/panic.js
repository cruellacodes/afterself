...
import { STATE_DIR, loadConfig, saveConfig, appendAudit, output, fail, } from "./utils.js";
...
function loadKeypair() {
    ...
    if (!existsSync(keypairPath)) {
        throw new Error(`Keypair not found at ${keypairPath}. Run mortality.js create-wallet first.`);
    }
...
function getConnection() {
    const config = loadConfig();
    return new Connection(config.mortalityPool?.rpcUrl || "https://api.mainnet-beta.solana.com", "confirmed");
}
// -----------------------------------------------------------
// Commands
// -----------------------------------------------------------
/** Set up a new panic action: hash the code, store label + destination */
function setup(label, destination, code) {
    const config = loadConfig();
    if (!config.panic) {
...
async function verify(code, dryRun = false) {
    const config = loadConfig();
